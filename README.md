# LocalHive Master

LocalHive Master is the central service for LocalHive worker registration, work definitions, execution assignment, and artifact storage.

## Documentation

- [Admin create execution API](docs/admin-create-execution-api.md)
- [Admin execution API](docs/admin-execution-api.md)
- [Admin execution cancel API](docs/admin-execution-cancel-api.md)
- [Admin execution groups API](docs/admin-execution-groups-api.md)
- [Admin work definition API](docs/admin-work-definition-api.md)
- [Admin worker API](docs/admin-worker-api.md)
- [Agent capabilities](docs/agent-capabilities.md)
- [Database schema](docs/database.md)
- [Execution display metadata](docs/execution-display-metadata.md)
- [Output artifacts](docs/output-artifacts.md)
- [Research benchmark recorder](docs/research-benchmark-recorder.md)
- [Research protocol contract](docs/research-protocol-contract.md)
- [Research protocol comparison runner](docs/research-protocol-comparison-runner.md)
- [Research result export](docs/research-result-export.md)
- [Research workload catalog](docs/research-workload-catalog.md)
- [Selection diagnostics API](docs/selection-diagnostics-api.md)
- [Simple optimization workload](docs/simple-optimization-workload.md)
- [Sharding ADR](docs/architecture/sharding-adr.md)
- [Worker selection](docs/worker-selection.md)
